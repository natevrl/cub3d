...
static void	malloc_struct(t_mlx *root)
{
	...
	if (root->walls == 0)
		malloc_error(root);
	root->collectible = malloc(sizeof(t_tuple));
	if (root->collectible == 0)
		malloc_error(root);
	root->enemies = malloc(sizeof(t_tuple));
	if (root->enemies == 0)
		malloc_error(root);
}
...
static void	init_struct(t_mlx *root)
{
	root->is_escape = 0;
	root->is_player = 0;
	root->collectible->max = 0;
	root->collectible->touched = 0;
	root->walls->max = 0;
	root->win_width = 0;
	root->win_height = 0;
	root->mlx = 0;
	...
	root->walls->y = 0;
	root->walls->x = 0;
	root->collectible->x = 0;
	root->collectible->y = 0;
	root->enemies->max = 0;
	root->enemies->touched = 0;
	root->enemies->x = 0;
	root->enemies->y = 0;
}
...
void	game_driver(char *path)
{
	...
	root->mlx_win = mlx_new_window(root->mlx, IMG_BITS * root->win_width,
			IMG_BITS * root->win_height, "so_long");
